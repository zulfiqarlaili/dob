...
const triD = (
  ...
  `M${a.x.toFixed(1)} ${a.y.toFixed(1)}L${b.x.toFixed(1)} ${b.y.toFixed(
    1
  ...
  return (
    <div className='star-chart-wrapper'>
      ...
      >
        ...
        <g filter='url(#sg)'>
          <path
            d={triD(U_TOP, U_BL, U_BR)}
            fill='none'
            stroke='rgba(197,180,237,0.55)'
            strokeWidth='2.5'
            strokeLinejoin='round'
          />
          <path
            d={triD(D_BOT, D_TL, D_TR)}
            fill='none'
            stroke='rgba(197,180,237,0.55)'
            strokeWidth='2.5'
            strokeLinejoin='round'
...
export function drawStarChartToCanvas(
  ...
  ctx.strokeStyle = 'rgba(197,180,237,0.55)';
  ctx.lineWidth = 2.5;
  ctx.lineJoin = 'round';
  ...
  numbers.forEach((num, i) => {
    ...
    if (isCore) {
      ...
      ctx.stroke();
      if (LABELS[i]) {
        ctx.fillStyle = '#b3bbcb';
        ctx.font = '500 13px Inter,sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(LABELS[i], p.x + 30, p.y + 2);
        ctx.textAlign = 'center';
      }
    }
    ctx.fillStyle = color;
    ctx.font = `700 ${isCore ? 32 : 30}px "Space Grotesk",Inter,sans-serif`;
  ...
  counts.forEach((el, i) => {
    ...
    ctx.beginPath();
    ctx.arc(35 + xOff, y, 8, 0, Math.PI * 2);
    ctx.fillStyle = el.color;
    ctx.fill();
    ctx.fillStyle = 'rgba(241,245,249,0.85)';
    ctx.font = '600 15px Inter,sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText(el.name, 52 + xOff, y + 1);
    ctx.fillStyle = el.color;
    ctx.font = '700 15px Inter,sans-serif';
    ctx.fillText(`× ${el.count}`, 120 + xOff, y + 1);
    ctx.textAlign = 'center';
  });
